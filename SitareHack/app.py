...
class Schedule(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    pid = db.Column(db.Integer, db.ForeignKey('professor_data.p_id'), nullable=False)
    day = db.Column(db.Text(100), nullable = False)
    course = db.Column(db.Text(1000), nullable = False)
    time = db.Column(db.Text(1000), nullable=False)
    type = db.Column(db.Text(100), nullable=False)
    semester = db.Column(db.String(50), nullable=False)
...
def professor_dashboard(name, pid):
    ...
    for i in sems:
        ...
        files[i] = files_temp
    schedule = Schedule.query.filter_by(pid=pid).all()
    return render_template('prof_dashboard.html', name=name, course=cdetail, student_lst=student_lst, files=files, img=prof_obj.img, schedule=schedule)
...
def student_dashboard(name, rollno, semester):
    ...
    for announcement in ann:
        ...
        count += 1
    schedule = Schedule.query.filter_by(semester=semester).all()
    return render_template('student_dashboard.html', name=name, rollno=rollno, email=student_obj.email, semester=semester, courses=courselst, img=student_obj.img, marks=marks_dict, announcements=announcements, schedule=schedule)
...
def add_schedule():
    if request.method == 'POST':
        ...
        typeschedule = request.form['type']
        obj = pcourse.query.filter_by(course=course).first()
        pid = obj.p_id
        sem = pcourse.query.filter_by(p_id=pid, course=course).first().semester
        new_schedule = Schedule(pid=pid, day=day, course=course, time=time, type=typeschedule, semester=sem)
        db.session.add(new_schedule)
        db.session.commit()
        flash('Schedule successfully added', 'success')
        return redirect(request.referrer)
    flash("Please re-try", 'error')
    return redirect(request.referrer)
...
### DELETE THE SCHEDULE FROM THE TABLE ###
